...
interface FilterConfig {
  salt: string;
  kAnonymityThreshold: number; // minimum independent sessions before contributing
}
 ...
 */
export function generaliseError(errorText: string): string {
  // Extract error type (e.g. "TypeError", "ENOENT", "404 Not Found")
  const errorTypeMatch = errorText.match(
    /\b((?:[A-Z][a-z]+)+Error|E[A-Z]+|[45]\d{2}\s+\w[\w ]*|\w+Exception)\b/
  );
 ...
 */
export function hashWithSalt(value: string, salt: string = DEFAULT_CONFIG.salt): string {
...
export function checkKAnonymity(
  ...
  k: number = DEFAULT_CONFIG.kAnonymityThreshold
): boolean {
 ...
 */
function normaliseApproach(text: string): string {
  return text
    .toLowerCase()
    .replace(/[^\w\s]/g, ' ')
    .replace(/\s+/g, ' ')
 ...
 */
export function toAnonymousPattern(
  entry: KnowledgeEntry,
  allEntries: KnowledgeEntry[],
  ...
  const patternHash = hashWithSalt(
    `${category}:${normaliseApproach(approach)}`,
    cfg.salt
  );
...
function isTechnologyTag(tag: string): boolean {
