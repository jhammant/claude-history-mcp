...
async function loadState(): Promise<FederationState> {
  try {
...
async function saveState(state: FederationState): Promise<void> {
  await mkdir(CONFIG_DIR, { recursive: true });
  await writeFile(STATE_FILE, JSON.stringify(state, null, 2) + '\n');
}

export async function federationCommand(args: string[]): Promise<void> {
...
async function handleContribute(): Promise<void> {
  const state = await loadState();
