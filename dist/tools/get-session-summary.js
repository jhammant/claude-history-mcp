...
export const getSessionSummaryTool = {
    ...
    description: "Get a structured summary of a conversation session. Provide either a session ID or project name (returns most recent session for the project).",
    inputSchema: {
        type: "object",
        properties: {
            session_id: {
            ...
            project: {
                type: "string",
                description: "Project name or path. Returns summary of most recent session.",
            },
        },
    },
};
...
function findLatestSessionForProject(projectQuery) {
...
function extractKeyPhrases(text) {
    ...
    // Error-like patterns
    const errors = text.match(/(?:error|Error|ERROR|exception|Exception|failed|FAILED)[:\s][^\n.]{5,80}/g);
    if (errors)
        phrases.push(...errors.slice(0, 3));
