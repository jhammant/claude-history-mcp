...
export const findPatternsTool = {
    name: "find_patterns",
    description: "Discover recurring patterns in conversation history: common topics, frequent workflows, repeated issues.",
    inputSchema: {
        type: "object",
        properties: {
            project: {
...
export function handleFindPatterns(args) {
    ...
    if (args.project) {
        ...
        filtered = entries.filter((e) => e.project.toLowerCase().includes(query) ||
...
function findTopicPatterns(entries) {
...
function findActivityPatterns(entries) {
    const patterns = [];
    ...
    patterns.push(`Most active day: ${dayNames[busiestDay]} (${dayCounts[busiestDay]} messages)`);
    const busiestHour = hourCounts.indexOf(Math.max(...hourCounts));
    patterns.push(`Most active hour: ${busiestHour}:00 (${hourCounts[busiestHour]} messages)`);
    // Average session length
    const bySession = groupBySession(entries);
...
function findRepeatedIssues(entries) {
