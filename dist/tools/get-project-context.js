import { parseHistoryFile, groupBySession } from "../parsers/history-parser.js";
import { extractProjectName } from "../utils/path-encoder.js";
export const getProjectContextTool = {
    name: "get_project_context",
    description: "Get comprehensive context for a project: recent sessions, key topics, common tools, and patterns. Useful for session-start context injection.",
    inputSchema: {
        type: "object",
        properties: {
            project: {
                type: "string",
                description: "Project name or path. Defaults to current working directory.",
            },
            depth: {
                type: "string",
                enum: ["brief", "normal", "detailed"],
                description: "Detail level: brief (last session only), normal (last 3), detailed (last 5 + patterns). Default: normal.",
            },
        },
    },
};
export function handleGetProjectContext(engine, args) {
    ...
    const matching = entries.filter((e) => e.project.toLowerCase().includes(query) ||
    ...
    for (const session of sessionList.slice(0, sessionCount)) {
        ...
        const daysAgo = Math.floor((Date.now() - session.lastActivity) / 86400000);
        ...
        const topic = firstMsg.length > 120 ? firstMsg.slice(0, 120) + "..." : firstMsg;
